...
public class TakingPhoto : Element
{
    ...
    {
        // PhotoManager 초기화
        PhotoManager.Init(ImaginationManager.GetActiveImageByID(_imaginationID));

        Debug.Log("사진 촬영 모드에 들어갑니다.");

        // 사진 촬영이 완료되거나 취소될 때까지 기다리는 루프
        while (!PhotoManager.IsPhotoTaken() && !PhotoManager.IsPhotoCancelled())
        {
        ...
        if (PhotoManager.IsPhotoTaken())
        {
            Debug.Log("사진 촬영이 완료되었습니다.");
        }
        else if (PhotoManager.IsPhotoCancelled())
        {
