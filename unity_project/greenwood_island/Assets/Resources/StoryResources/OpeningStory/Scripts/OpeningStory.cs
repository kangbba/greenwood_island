    ...
    (
        ...
        new Dialogue(
            ...
            new List<Line>
            {
                ...
                new Line(EEmotionID.Normal, 0, "창문 너머로 보이는 하늘은 회색으로 뒤덮여 있고, 두려움이 나를 감싸기 시작했다."),
                new Line(EEmotionID.Normal, 0, "애써 의연한 척 해보려 고개를 빼어 이 작은 여객선을 둘러본다."),
                new Line(EEmotionID.Normal, 0, "어두운 분위기, 이 작은 배에는 나 외에 몇 명밖에 없다."),
                new Line(EEmotionID.Normal, 0, "몇 명의 승객들이 고개를 떨군 채 침묵하고 있다. 내가 기댈 만한 사람이라고는 없어 보인다."),
            }
        ),
        new CameraShake(),
        ...
        new Dialogue(
            ...
            new List<Line>
            {
                ...
                new Line(EEmotionID.Normal, 0, "흔들리는 배에도 두려움이 없어보인다. 그렇다고 강인한 모습은 아닌 것 같다."),
                new Line(EEmotionID.Normal, 0, "바다만 바라보며 그저 혼잣말을 반복할 뿐이었다"),
            }
        ),
        ...
        new Dialogue(
            ...
            new List<Line>
            {
                new Line(EEmotionID.Normal, 0, "바다가… 다 기억해… 내가 본 것들… 물속에 잠겼어… 저 깊은 곳에…"),
            }
        ),
        new CameraShake(),
        new CameraShake(3),
        new Dialogue(
            ...
            new List<Line>
            {
                new Line(EEmotionID.Normal, 0, "나는 그저 이 배를 흔드는 태풍이 조금 잠잠해지기를 바랄 뿐이다."),
            }
        ),
        // 라디오의 음성
        new Dialogue(
        ...
        new Dialogue(
            ...
            new List<Line>
            {
                ...
                new Line(EEmotionID.Normal, 0, "평화로운 해안과 오래된 건물들이 마치 시간이 멈춘 듯 라이언의 눈앞에 펼쳐진다."),
                new Line(EEmotionID.Normal, 0, "방금 전의 긴장은 거짓말처럼 사라지지만, 남은 여운은 가시지 않는다."),
                new Line(EEmotionID.Normal, 0, "섬에 도착했지만, 어딘지 모르게 편치 않다."),
